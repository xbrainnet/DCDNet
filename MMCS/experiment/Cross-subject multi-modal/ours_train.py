...
from models.My2CrossNet import MyCCNet
...
def train(model, train_loader, val_loader, test_loader, modalities,
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch_index, (source_x, source_y) in enumerate(train_loader):
            ...
            target_x_list = divide(target_x, modalities)
...
if __name__ == '__main__':
    ...
    x_train_fold, y_train_fold, x_val_fold, y_val_fold, x_test_fold, y_test_fold = My_Model_Special_processdata3(path, fold,
                                                                                                             batch=batch,
    ...
    for i in range(fold):
        ...
        model = MyCCNet(input_size, sample_num, d_model, num_AttLayers=num_AttLayers, num_heads=num_heads, dropout=dropout)
        model.to(device)
    ...
    save_path = '../../save/3/Oursss.npy'
    np.save(save_path, save_data)

    save_best_data = np.concatenate(bestRes)
    save_best_path = '../../save/3/Ours_besttt.npy'
